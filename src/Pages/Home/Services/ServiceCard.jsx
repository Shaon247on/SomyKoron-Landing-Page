...
const ServiceCard = ({ number, icon, title, subtitle, iconBGColor, position, border,lastNumber}) => {
    return (
        <div className={`col-span-4 lg:col-span-1 relative ml-8 ${position}`}>
            ...
            <p className={`${iconBGColor} p-4 rounded-full w-[65px] z-30 relative`}>
                {icon}
            </p>
            {
                border && <div className="border-dashed border-t-2 border-r-2 rounded-tr-lg absolute top-[120px] left-[40px] z-10 border-[#647296] w-[316px] h-[66px]"/>
            }
